...
**Tier 2 — `keyword`.** Regex match on ~25 patterns across company name + job title. Catches roles at non-named companies that fit the target shape: forward-deployed / applied-AI / solutions engineering, BizOps / strategy / GTM / product analytics, defense and national security, and APM / program-management. Quant, trading, and data-science keywords were pruned in the June 2026 keyword cleanup. Before the keyword check, `SKIP_TITLE_PATTERNS` drops obviously unwinnable titles (SWE, ML engineer, data scientist, quant, data engineer, embedded, etc.) — named-target companies bypass the skip filter.
